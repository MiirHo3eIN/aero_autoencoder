...
def calculateMSE(md, results):
    ...
    output = mseOverall(x.float(), x_hat.float()).item()
    results["msePerSensor"] = msePerSensor(x.float(), x_hat.float()).tolist()
    ...
    results["siganls"] = torch.cat((x[[idx]], x_hat[[idx]]))
